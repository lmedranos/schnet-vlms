...
class SpkCalculator(Calculator):
    ...
    aprop = Properties.aprop
    implemented_properties = [energy, forces, stress, aprop]
...
#                    energy.item() * self.energy_units
            ...
            if self.model_aprop is not None:
                aprop = model_results[self.model_aprop].cpu().data.numpy()
                results[self.aprop] = (
                    aprop.reshape((1, 3))
                )
